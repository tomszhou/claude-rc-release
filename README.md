...
```bash
claude-rc start --model sonnet              # 指定模型
claude-rc start --model opus --permission-mode plan   # 模型 + 权限模式
claude-rc start --add-dir /tmp/data /var/logs         # 额外目录
claude-rc start --allowed-tools "Bash(git:*)" Edit    # 限制工具
```
